  ...
  try {
    ...
    if (data && expectedResponse && expectedResponse.data) {
      ...
      return {
        ...
        name: '${(example.name || example.id).replace(/'/g, "\\'")}',
        passed: dataMatches,
        error: null,
        data: data,
...
processTestSuite().then(tests => {
  ...
  // Write test manifest
  fs.writeFileSync('test-manifest.json', JSON.stringify(tests, null, 2));
  console.log(`📋 Test manifest saved to test-manifest.json`);
