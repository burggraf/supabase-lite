...
// Detect if we're running in a deployed context (App Hosting)
function isDeployedContext(): boolean {
  if (typeof window === 'undefined') return false;
  
  // Check if we're running at /app/* path (App Hosting)
  return window.location.pathname.startsWith('/app/');
}

// PostMessage-based API call for deployed contexts
async function executeAPIViaPostMessage(
  method: string,
  endpoint: string,
  headers: Record<string, string>,
  body?: any
  ...
  return new Promise((resolve) => {
    ...
    const responseHandler = (event: MessageEvent) => {
      console.log('📥 Received PostMessage response:', { origin: event.origin, type: event.data?.type, requestId: event.data?.data?.requestId, source: event.source === window ? 'self' : 'parent' });
      
      // Prevent handling our own messages - only accept from parent window
      if (event.source === window) {
        console.log('🔄 Ignoring message from self');
        return;
      }
      
      // Only accept messages from same origin (Supabase Lite)
      if (event.origin !== window.location.origin) {
        console.log('❌ Origin mismatch:', { received: event.origin, expected: window.location.origin });
        return;
      }
      
      const { type, data } = event.data;
      
      // CRITICAL FIX: Only handle API_RESPONSE messages, ignore API_REQUEST (our own messages)
      if (type !== 'API_RESPONSE') {
        console.log('🔄 Ignoring non-response message type:', type);
        return;
      }
    ...
    console.log('📤 Sending PostMessage to parent:', { method, endpoint, requestId });
    ...
    }, '*'); // Use '*' for same-origin deployed apps
...
export async function executeAuthTest(test: AuthTest, options: { skipAutoAuth?: boolean } = {}): Promise<AuthResponse> {
  ...
  try {
    ...
    // Check if we're in a deployed context and use PostMessage API instead of fetch
    if (isDeployedContext()) {
      console.log('🏠 Deployed context detected, using PostMessage API for:', test.endpoint);
      ...
      const response = await executeAPIViaPostMessage(
        test.method,
        test.endpoint,
        headers,
        requestBody
      ...
      if (response.status >= 200 && response.status < 300 && response.data && 
        (test.id.includes('signin') || test.id.includes('signup') || test.id.includes('refresh') ||
         test.id.includes('auto-signin') || test.id.includes('auto-signup'))) {
        storeAuthData(response.data);
      }

      // Clear auth data on logout
      if (response.status >= 200 && response.status < 300 && test.id.includes('logout')) {
        clearAuthData();
      }
    ...
    // Regular fetch-based execution for standalone mode
